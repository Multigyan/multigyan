...
class RedisCache {
  ...
  async deletePattern(pattern) {
    ...
    try {
      ...
      console.log(`✅ Redis DELETE PATTERN: ${pattern} (${keys.length} keys)`)
      return keys.length
    } catch (error) {
  ...
  async flush() {
    ...
    try {
      await redis.flushdb()
      console.log('✅ Redis FLUSH: All keys deleted')
      return true
    } catch (error) {
  ...
  async increment(key, amount = 1) {
    ...
    try {
      ...
      console.log(`✅ Redis INCREMENT: ${key} by ${amount} = ${result}`)
      return result
    } catch (error) {
  ...
  async setNX(key, value, ttl = 300) {
    ...
    try {
      ...
      if (result === 'OK') {
        console.log(`✅ Redis SETNX: ${key} (TTL: ${ttl}s)`)
        return true
      }
      
      console.log(`❌ Redis SETNX FAILED: ${key} already exists`)
      return false
    } catch (error) {
