...
export function generateSEOMetadata({
  ...
  // ✅ FIX: Generate optimized title that respects Bing's 70-character limit
  // For articles, use absolute title (no suffix) to stay under 70 chars
  // For other pages, add suffix only if it fits
  let fullTitle

  if (type === 'article') {
    // Articles: Use absolute title (already optimized by author)
    fullTitle = title || `${siteName} - Multi-Author Blogging Platform`
  } else {
    // Other pages: Add suffix if total length <= 70 characters
    if (title) {
      const titleWithSuffix = `${title} | ${siteName}`
      fullTitle = titleWithSuffix.length <= 70 ? titleWithSuffix : title
    } else {
      fullTitle = `${siteName} - Multi-Author Blogging Platform`
    }
  }
