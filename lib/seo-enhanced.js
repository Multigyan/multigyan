 ...
 * Preserves paragraph structure and removes scripts/styles
...
export function generateArticleSchema(post) {
  ...
  // ✅ NOTEBOOKLM FIX: Extract clean text content for articleBody
  const cleanContent = extractCleanText(post.content)
  const contentSnippet = cleanContent.substring(0, 300) + (cleanContent.length > 300 ? '...' : '')
  ...
  return {
    ...
    // ✅ CRITICAL FIX: Add full article body for AI crawlers like NotebookLM
    articleBody: cleanContent,

    // ✅ Add short abstract for previews
