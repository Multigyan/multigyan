...
export function generateArticleSchema(post) {
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://multigyan.in'
  const language = post.lang || 'en'  // Changed from post.language
  ...
  return {
    ...
    image: post.featuredImageUrl || `${baseUrl}/Multigyan_Logo_bg.png`,
    datePublished: post.publishedAt || post.createdAt,
    dateModified: post.updatedAt || post.publishedAt || post.createdAt,
    author: {
      '@type': 'Person',
      name: post.author?.name || 'Multigyan Author',
      url: post.author?.username ? `${baseUrl}/author/${post.author.username}` : baseUrl
    },
    publisher: {
      '@type': 'Organization',
      name: 'Multigyan',
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `${baseUrl}${langPrefix}/blog/${post.slug}`
    },
...
export function generateSEOMetadata({
  ...
  const metadata = {
    ...
    openGraph: {
      ...
      siteName: 'Multigyan',
      locale: language === 'hi' ? 'hi_IN' : 'en_IN',  // Changed from post.language
      type: ogType,
      images: ogImage ? [
        {
          url: ogImage,
