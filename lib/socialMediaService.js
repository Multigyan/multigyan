...
class SocialMediaService {
    ...
    /**
     * Post to LinkedIn
     * Supports both personal profile and organization posting
     */
    async postToLinkedIn(post) {
        try {
            ...
            const hasOrgId = this.credentials.linkedin.organizationId
            const authorUrn = hasOrgId
                ? `urn:li:organization:${this.credentials.linkedin.organizationId}`
                : `urn:li:person:me` // Use 'me' for personal profile
            ...
            if (!response.ok) {
                const error = await response.json()
                throw new Error(`LinkedIn API error: ${error.message || error.error || JSON.stringify(error)}`)
            }
